...
public class Game {
  ...
  public boolean insertDisc(int columnNumber)
    throws ColumnIsFullException, InvalidColumnException {
    Disc disc;
  ...
  public boolean checkWin(int rowNumber, int columnNumber, Player turnPlayer) {
    ...
    count = 0;
    for (
      int i = rowNumber, j = columnNumber;
      i < numberOfRows && j < numberOfColumns;
      i++, j++
    ) {
      if (board[i][j] == null) {
        break;
